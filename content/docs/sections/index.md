...
По умолчанию, если для установки использовался скрипт, создается одна секция - **Main**.
...
При выбранном режиме `Proxy` доступна еще одна настройка - **UDP over TCP**,которая используется для передачи UDP-пакетов в TCP соединении. Если UDP заблокирован, а TCP - нет, то данная опция может помочь.
Возможно использовать с `Shadowsocks` и `Socks5`.
...
Позволяет указать несколько ссылок `vless://  ss:// trojan:// socks(4|5)://`. В данном режиме `sing-box`  отправляет запрос к указанному серверу и измеряет скорость отклика, наличие ошибок и другие параметры соединения и на основании данных тестов выбирает сервер. В данный момент, при наличии нескольких действующих серверов, переключение произойдет только в случае если значение задержки текущего активного сервера больше другого на 50мс.
...
<!-- Создать раздел Дашборда, если путь будет отличаться-исправить -->
...
резолвера доменов аналогична [обычной настройке DNS-сервера](/docs/base-settings/#тип-протокола-dns-dns-protocol-type).
...
Позволяет поднять на роутере `SOCKS` прокси и направлять весь трафик с определенного приложения на роутере или в локальной сети в прокси указанный в данной секции.

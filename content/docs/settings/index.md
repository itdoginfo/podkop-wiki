...
##### Включить доступ YACD WAN | Enable YACD WAN Access
...
## Скачивать списки через Proxy/VPN | Download Lists via Proxy/VPN

Позволяет загружать списки через Proxy или VPN, а не напрямую.

Эта опция может быть полезна, если доступ к GitHub или другим источникам списков ограничен или заблокирован.

## Скачивать списки через выбранную секцию | Download Lists via specific proxy section

Позволяет выбрать секцию (туннель), через которую будут загружаться списки.
...
Задает уровень логирования для sing-box.

Более высокий уровень логирования позволяет получить больше диагностической информации, но может увеличить объем логов и нагрузку на систему. Рекомендуется изменять эту настройку только при необходимости отладки или анализа работы сервиса.
...
## Исключенные из маршрутизации IP-адреса | Routing Excluded IPs

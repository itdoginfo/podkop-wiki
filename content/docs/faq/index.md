...
#### Как исключить домен (например 4pda.to) из предустановленного списка доменов (например Russia-inside) для отправки в туннель?
 Для того, чтобы исключить домен, который не должен направляться в туннель:
- Создайте новую секцию, выбрав в качестве **Connection Type - Exclusion**
- Домены, которые вам необходимо исключить, добавьте как [пользовательские домены](/docs/sections/#tip-polzovatelskogo-spiska-domenov-user-domain-list-type)
...
#### Как использовать WARP и пользовательский список подсетей Cloudflare?

При использовании **WG | AWG** подключения с WARP и пользовательского списка подсетей **Cloudflare** в другом туннеле, происходит зацикливание. Чтобы избежать этого укажите в дополнительных настройках **WG | AWG** интерфейса с WARP в поле **Firewall Mark** значение 0x00200000. 
...
0.0.0.0         100.122.0.1     0.0.0.0         UG    0      0        0 wan
```
В первой строке и столбце **Iface** будет указано необходимое имя интерфейса.

Создайте новую секцию в Podkop с типом подключения **VPN**.
...
```
Не обращайте внимание на то, что рядом с интерфейсом будет указано **Absent Interface**.
...
## Как исправить ошибку подключения после обновления на сервере версии 3x-ui/Xray?

Разработчик **Xray-core**, начиная с версии 26.7.11, внес изменение, которое сломало работу многих клиентов, в том числе и работу **Sing-box**.

Чтобы исправить это вам нужно указать **minClientVer** на сервере.

В случае с **3X-UI** перейти в настройки **Inbounds → Security → REALITY → Min Client Ver (Изменить подключение -> Безопасность -> Reality -> Мин. версия клиента)** и указать 1.8.0.

Для Xray или панелей, где в качестве конфигурации ядра используется json, вам необходимо указать её в разделе **realitySettings**
```shell
"inbounds": [
      ...
      "security": "reality",

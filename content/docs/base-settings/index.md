...
Позволяет указать отдельный DNS-сервер при использовании Wireguard, AmneziaWG, OpenConnect, OpenVPN, который будет использоваться для доменов из списка для обхода.

Подробнее [здесь](/docs/dns).

##### Split DNS Protocol Type

Позволяет указать протокол при использовании Split DNS. 
...
Позволяет указать или выбрать из списка DNS адрес, который будет использоваться для **Split DNS**
...
Подробнее о настройке вы можете узнать [здесь](/docs/donttouchmydhcp).

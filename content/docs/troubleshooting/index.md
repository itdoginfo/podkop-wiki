...
> *   В диагностике ошибка `FakeIP Status: ! does not work in browser`? Скорее всего, дело в **[DNS на устройстве](#диагностика-на-клиентском-устройстве-пк-смартфон)** или **[«Безопасном DNS» в браузере](#windows-macos-linux-—-открываем-терминал-для-ввода-команд)**.
...
5.  **Правила `netfilter`** активны, корректно маркируют транзитный трафик из локальной сети и перенаправляют его через `tproxy` на сокет `127.0.0.1:1602`.
...
| | ! `check error` | Запрос из браузера не удался. FakeIP-адрес получен, но трафик не доходит до `sing-box`. Проверьте, является ли роутер шлюзом для устройства, и нет ли ошибок в правилах `nft`. |
| **Config** | ✔ `working` | Туннель работает. Запросы к диагностическим доменам вернули разные IP-адреса. |
| | ✘ `same IP for both domains`| Туннель работает, но оба запроса прошли через него. Возможные причины: IP устройства включен в «Принудительное проксирование» или весь трафик заворачивается через VPN-интерфейс. |
...
1.  **Проверка получения FakeIP:**
    ...
    *   **Если в ответе другой IP** (например, `64.188.104.86` вместо `198.18.x.x`), значит, ваше устройство использует сторонний DNS (его адрес в можно увидеть в строке Server). Решение — в статье [«DNS на устройствах»](/docs/client-dns/).
...
2.  **Проверка прохождения трафика через `sing-box`:**
    ...
    *   **Если `curl` подключается, но в ответе `"fakeip": false`**, это указывает на серьёзную проблему в работе `sing-box` или правил `netfilter`.
...
Если на устройстве есть браузер с поддержкой JavaScript, откройте в нём веб-интерфейс роутера (LuCI) и перейдите на вкладку «Диагностика». Этой информации, как правило, достаточно для понимания проблемы.
...
1.  **Проверка, какие службы прослушивают порты:**
    ...
    ```text
    tcp        0      0 127.0.0.42:53           0.0.0.0:*               LISTEN      30933/sing-box
    tcp        0      0 127.0.0.1:1602          0.0.0.0:*               LISTEN      30933/sing-box
    tcp        0      0 127.0.0.1:53            0.0.0.0:*               LISTEN      31296/dnsmasq
    tcp        0      0 192.168.1.1:53          0.0.0.0:*               LISTEN      31296/dnsmasq
    ```
...
*   **Сумма пакетов** в цепочках `mangle` и `mangle_output` должна примерно соответствовать счетчикам в цепочке `proxy`. Если в `mangle` пакеты есть, а в `proxy` — нули, значит, трафик маркируется, но не перенаправляется в `sing-box`. Это может быть вызвано конфликтом с другими правилами `netfilter` (например, от службы `vpn-client` на прошивках GL.iNet).

## Проверка работоспособности VPN-интерфейсов
...
## Проверка работоспособности прокси-ссылки
